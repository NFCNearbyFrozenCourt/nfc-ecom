    ...
    try {
        ...
        foreach ($cartItems as $item) {
            try {
                ...
                if ($pricingGroupId) {
                    if ($productInfo->price_inc_tax !== null) {
                        // Use the pricing group's price if available
                        $unitPrice = $productInfo->price_inc_tax;
                        $selling_price = $productInfo->price_inc_tax;
                    } else if ($productInfo->price_type === 'percentage' && $productInfo->price_inc_tax === null) {
                        // Calculate percentage-based discount only if no direct price is set
                        $percentageValue = $productInfo->price_type === 'percentage' ? $productInfo->price_type : 0;
                        $unitPrice = $productInfo->default_sell_price * (1 + ($percentageValue / 100));
                ...
                $lineTotalWithTax = $lineTotal + ($itemTax * $item->quantity);
        ...
        $transactionId = DB::table('transactions')->insertGetId([
            ...
            'final_total' => $totalAmount - $totalTax,
        ...
        try {
            ...
            $formattedProducts = $products->map(function ($product) {
                return [
                    ...
                    'final_price' => $product->variation->sell_price_inc_tax ?? $product->regular_price,
                ];
            });
