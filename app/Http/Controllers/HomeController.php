     ...
     private function extractPincode($address)
{
    ...
    public function index()
    {
        ...
        if(!$user){
            ...
            return view('welcome', compact( 'products','shippingAddress'));
        }
        ...
        if ($user->user_type == 'user_customer') {
            ...
            if (!$pincode) {
                ...
                $products = Product::all();
                return view('welcome', compact('user', 'products','shippingAddress'));
            }
            
            // Step 2: Check if location exists for the given pincode in ecom_delivery_locations
            ...
            // Step 3: Get the selling_price_group_id from business_locations
...
// Process the products to calculate final prices
foreach ($products as $product) {
    if ($product->price_type === 'percentage' && $product->price_inc_tax !== null) {
        ...
        // Fall back to regular price if no group price is available
        $product->final_price = $product->regular_price;
    }
}
            // Step 5: Return view with fetched products
            return view('welcome', compact('user', 'products','shippingAddress'));
        }
