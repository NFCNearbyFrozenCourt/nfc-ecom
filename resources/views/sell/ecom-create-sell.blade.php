...
<script>
// CSS to transform the table into cards
const css = `
  /* Mobile view styles */
  @media (max-width: 768px) {
    ...
    /* Style each cell as a flex container with row direction */
    #pos_table tbody td {
      display: flex;
      padding: 8px 10px;
    ...
    /* Hide serial number */
    #pos_table tbody td.serial_no {
      display: none;
    }
    
    /* Create label for each field and position left */
    #pos_table tbody td:nth-child(3):before { 
      content: "Quantity: "; 
      font-weight: bold; 
      margin-right: -25px; 
      min-width: 120px; 
    ...
    #pos_table tbody td:nth-child(4):before { 
      content: "Unit Price: "; 
      font-weight: bold; 
      margin-right: -25px; 
      min-width: 120px; 
    ...
    #pos_table tbody td:nth-child(5):before { 
      content: "Discount: "; 
      font-weight: bold; 
      margin-right: -25px; 
      min-width: 120px; 
    ...
    #pos_table tbody td:nth-child(6):before { 
      content: "Tax: "; 
      font-weight: bold; 
      margin-right: -25px; 
      min-width: 120px; 
    ...
    #pos_table tbody td:nth-child(7):before { 
      content: "Price inc. tax: "; 
      font-weight: bold; 
      margin-right: -25px; 
      min-width: 120px; 
      flex-shrink: 0;
    }
    #pos_table tbody td:nth-child(8):before { 
      content: "Subtotal: "; 
      font-weight: bold; 
      margin-right: -25px; 
      min-width: 120px; 
    ...
    /* Position the remove button in top right corner */
    ...
    /* Style product cell - simplified to only show image and product name */
    ...
    /* Extract and show only product name */
    #pos_table tbody td:nth-child(2) {
      position: relative;
    }
    
    /* Hide all elements except image and first text node in product cell */
    #pos_table tbody td:nth-child(2) input,
    #pos_table tbody td:nth-child(2) select,
    #pos_table tbody td:nth-child(2) textarea,
    #pos_table tbody td:nth-child(2) p,
    #pos_table tbody td:nth-child(2) small,
    #pos_table tbody td:nth-child(2) br {
      display: none !important;
    }
    
    /* Make product image more prominent */
    #pos_table tbody td:nth-child(2) img {
    ...
    /* Clean up the product name text */
    #pos_table tbody td:nth-child(2)::after {
    ...
    /* Special handling for the quantity cell to keep inline controls */
    #pos_table tbody td:nth-child(3) {
      flex-direction: row;
      flex-wrap: wrap;
    }
    
    /* Position the content to the right of the label */
    #pos_table tbody td:nth-child(3) .input-group.input-number {
      flex: 1;
      min-width: 60%;
    }
    
    /* Ensure the input group elements remain inline */
    ...
    #pos_table tbody td .input-group-btn {
      display: inline-block !important;
      width: auto !important;
    }
    
    #pos_table tbody td .input-group.input-number input {
      flex: 1;
      width: auto !important;
	  height:35px;
    }
    
    /* Style the dropdown to appear below the quantity but aligned with the content */
    ...
    /* Make inputs appear more mobile-friendly */
...
// Function to add the CSS to the page
...
// Extract product name and set it as a data attribute
function extractProductNames() {
  const productCells = document.querySelectorAll('#pos_table tbody td:nth-child(2)');
  
  productCells.forEach(cell => {
    // Get the text content of the cell
    const cellText = cell.textContent.trim();
    
    // Extract product name - assume it's the first line before any <br>
    let productName = cellText.split('\n')[0].trim();
    
    // If there's a product code (e.g., "0077"), remove it
    productName = productName.replace(/\s*\d{4,}\s*$/, '');
    
    // Set as data attribute
    cell.setAttribute('data-product-name', productName);
  });
}

// Apply these styles when the DOM is loaded
document.addEventListener('DOMContentLoaded', function() {
  addCardStyles();
  extractProductNames();
});

// Force card view for all screen sizes (optional)
const forceCardView = `
  #pos_table thead {
    display: none !important;
  }
  ...
  /* Style each cell as a row with label on left and content on right */
  #pos_table tbody td {
    display: flex !important;
    flex-direction: row !important;
  ...
  /* Put small text on next line in forced card view */
  #pos_table tbody td small {
    display: block !important;
    width: 100% !important;
  ...
  /* Style for small helper text in forced card view */
  #pos_table tbody td .help-block {
    display: block !important;
    width: 100% !important;
    margin-top: 5px !important;
  ...
  /* Apply all other card styles as needed */
  /* Force the same horizontal layout in forced card view */
  #pos_table tbody td .input-group.input-number {
    display: flex !important;
    flex-direction: row !important;
    flex: 1 !important;
...
// Uncomment the following code to force card view regardless of screen size
/*
document.addEventListener('DOMContentLoaded', function() {
  const styleEl = document.createElement('style');
  styleEl.textContent = forceCardView;
  document.head.appendChild(styleEl);
});
*/
</script>
...
<input type="hidden" id="item_addition_method" value="{{$business_details->item_addition_method}}">
	...
	<div class="row">
		<div class="col-md-12 col-sm-12">
			@component('components.widget', ['class' => 'box-solid'])
					...
					<div class="form-group">
						...
						<div class="input-group">
							...
							{!! Form::select('contact_id', 
								...
								$contact_id ?? null, 
								['class' => 'form-control mousetrap', 'id' => 'customer_id', 'placeholder' => 'Enter Customer name / phone']
							) !!}
				...
				<div class="col-sm-3">
					<div class="form-group">
						...
						{!! Form::select('sales_order_ids[]', [$invoice_no => $invoice_no], $invoice_no ?? null, ['class' => 'form-control select2', 'multiple', 'id' => 'sales_order_ids']); !!}
					</div>
				</div>
				<div class="clearfix"></div>
			@endif
			...
			@endcomponent
			@component('components.widget', ['class' => 'box-solid'])
				<div class="col-md-4  @if($sale_type == 'sales_order') hide @endif" style="display:none">
			        <div class="form-group">
			    ...
			    <div class="col-md-4 @if($sale_type == 'sales_order') hide @endif" style="display: none"><br>
			    ...
			    <div class="col-md-4 col-md-offset-4  @if($sale_type == 'sales_order') hide @endif" style="display: none">
...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Setup for automatic population from URL parameters
    setTimeout(() => {
        const urlParams = new URLSearchParams(window.location.search);
        const invoiceNo = urlParams.get('invoice_no');
        const contactId = urlParams.get('contact_id');
        console.log("Extracted URL params:", invoiceNo, contactId);

        // Step 1: Set Customer ID
        function setCustomerId() {
            if (contactId) {
                let customerSelect = $('#customer_id');
                console.log("Setting customer ID:", contactId);

                if (customerSelect.find(`option[value="${contactId}"]`).length === 0) {
                    customerSelect.append(new Option(contactId, contactId, true, true));
                }

                customerSelect.val(contactId).trigger('change');
                
                // This will trigger get_sales_orders() which loads the orders for this customer
                
                // Step 2: After setting customer and loading sales orders, select the invoice
                setTimeout(() => {
                    // Wait for sales orders to load before selecting the invoice
                    if (invoiceNo) {
                        // We need to find the numeric ID for the invoice number
                        let salesOrderSelect = $('#sales_order_ids');
                        let options = salesOrderSelect.find('option');
                        let found = false;
                        
                        options.each(function() {
                            if ($(this).text() === invoiceNo) {
                                let numericId = $(this).val();
                                console.log("Found matching order:", invoiceNo, "with ID:", numericId);
                                
                                salesOrderSelect.val(numericId).trigger('change');
                                salesOrderSelect.trigger({
                                    type: 'select2:select',
                                    params: {
                                        data: {
                                            id: numericId,
                        ...
                        if (!found) {
                            console.log("Invoice not found in loaded options. Waiting longer...");
                            // If not found, try again after a longer delay (sales orders might still be loading)
                            setTimeout(function() {
                                options = salesOrderSelect.find('option');
                                
                                options.each(function() {
                                    if ($(this).text() === invoiceNo) {
                                        let numericId = $(this).val();
                                        console.log("Found matching order on retry:", invoiceNo, "with ID:", numericId);
    ...
    // Override the original get_sales_orders function to add a callback
    var original_get_sales_orders = window.get_sales_orders;
    
    window.get_sales_orders = function(callback) {
