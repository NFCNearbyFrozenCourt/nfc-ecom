...
<!-- Main content -->
<section class="content no-print" style="background-color: #fff;">
    ...
    <div class="container">
        <div class="header">
            <h1>{{ __('sale.sell_details') }} (<b>{{ $sell->type == 'sales_order' ? __('restaurant.order_no') : __('sale.invoice_no') }}:</b> {{ $sell->invoice_no }})</h1>
            <p class="date"><b>{{ __('messages.date') }}:</b> {{ @format_date($sell->transaction_date) }}</p>
        </div>
        
        <div class="details-grid">
            <div class="details-box">
                <b>{{ $sell->type == 'sales_order' ? __('restaurant.order_no') : __('sale.invoice_no') }}:</b> #{{ $sell->invoice_no }}<br>
                <b>{{ __('sale.status') }}:</b> 
                ...
                @if(!empty($sales_orders))
                    <br><br><strong>{{ __('lang_v1.sales_orders') }}:</strong>
                    <div class="table-responsive">
                        <table class="table no-border">
                            <tr>
                                <th>{{ __('lang_v1.sales_order') }}</th>
                ...
                @if($sell->document_path)
                    ...
                    download="{{$sell->document_name}}" class="btn btn-primary btn-sm">
            ...
            <div class="details-box">
                ...
                @else
                    ...
                    @if($sell->contact->mobile)
                        <br>{{__('contact.mobile')}}: {{ $sell->contact->mobile }}
                    @endif
                    @if($sell->contact->alternate_number)
                        <br>{{__('contact.alternate_contact_number')}}: {{ $sell->contact->alternate_number }}
                    @endif
                    @if($sell->contact->landline)
                        <br>{{__('contact.landline')}}: {{ $sell->contact->landline }}
                    @endif
                    @if($sell->contact->email)
                        <br>{{__('business.email')}}: {{ $sell->contact->email }}
                    @endif
                @endif
            </div>
            ...
            <div class="details-box">
                @if(in_array('tables', $enabled_modules))
                    <strong>{{ __('restaurant.table') }}:</strong>
                    {{$sell->table->name ?? ''}}<br>
                @endif
                
                @if(in_array('service_staff', $enabled_modules))
                    <strong>{{ __('restaurant.service_staff') }}:</strong>
                    {{$sell->service_staff->user_full_name ?? ''}}<br>
                @endif
                
                <strong>{{ __('sale.shipping') }}:</strong>
                <span class="label @if(!empty($shipping_status_colors[$sell->shipping_status])) {{$shipping_status_colors[$sell->shipping_status]}} @else {{'bg-gray'}} @endif">
                ...
                @if(!empty($sell->delivered_to))
                    <br><strong>{{ __('lang_v1.delivered_to') }}: </strong> {{$sell->delivered_to}}
                @endif
                
                @if(!empty($sell->delivery_person_user->first_name))
                    <br><strong>{{ __('lang_v1.delivery_person') }}: </strong> 
                    {{$sell->delivery_person_user->surname}} {{$sell->delivery_person_user->first_name}} {{$sell->delivery_person_user->last_name}}
                @endif
                ...
                @if(count($sell->media->where('model_media_type', 'shipping_document')))
                    @include('sell.partials.media_table', ['medias' => $sell->media->where('model_media_type', 'shipping_document')])
                @endif
            </div>
        ...
        <div class="section">
            <h2 class="section-title">{{ __('sale.products') }}</h2>
            <div class="table-responsive" style="
            overflow: hidden;
            padding 10px;
        ">
                @include('sale_pos.partials.sale_llne_details_ecom')
            </div>
        </div>
        ...
        @if($sell->type != 'sales_order')
        <div class="section">
            <h2 class="section-title">{{ __('sale.payment_info') }}</h2>
            <div class="details-grid">
                <div class="details-box">
                    <div class="table-responsive">
                        <table class="table bg-gray">
                        ...
                        <table class="table bg-gray">
                            ...
                            <tr>
                                ...
                                <td>
                                    <div class="pull-right">
                                        <span class="display_currency" @if($sell->discount_type == 'fixed') data-currency_symbol="true" @endif>
                            ...
                            @if(in_array('types_of_service', $enabled_modules) && !empty($sell->packing_charge))
                                <tr>
                                    ...
                                    <td>
                                        <div class="pull-right">
                                            <span class="display_currency" @if($sell->packing_charge_type == 'fixed') data-currency_symbol="true" @endif>
                                                {{ $sell->packing_charge }}
                                            </span> 
        ...
        <div class="section">
            <h2 class="section-title">{{ __('lang_v1.activities') }}</h2>
            @includeIf('activity_log.activities', ['activity_type' => 'sell'])
        </div>
        
        <div class="button-group">
            @if($sell->type != 'sales_order')
                <a href="#" class="btn btn-success print-invoice" data-href="{{route('sell.printInvoice', [$sell->id])}}?package_slip=true">
                    <i class="fas fa-file-alt"></i> {{ __('lang_v1.packing_slip') }}
                </a>
            @endif
        </div>
        <div class="col-sm-12 text-center tw-mt-2">
			
			<a href="{{ action([\App\Http\Controllers\SellController::class, 'ecomcreate'], ['invoice_no' => $sell->invoice_no, 'contact_id' => $sell->contact->id]) }}" 
                class="tw-dw-btn tw-dw-btn-success tw-dw-btn-lg tw-text-white">
                 Convert to Sell
             </a>
		</div>
    </div>
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        var elements = document.querySelectorAll('.display_currency');
        for (var i = 0; i < elements.length; i++) {
            var element = elements[i];
            if (element.hasAttribute('data-currency_symbol')) {
                // This would be replaced with your actual currency formatting logic
